...
class UserService:
    ...
    def list_users_form(self, *, db: Session, skip: int = 0, limit: int = 100) -> List[UserSchema]:
        users_form = users_form_repo.get_all(db=db, skip=skip, limit=limit)
        return users_form

    def create_users_form(self, *, db: Session, users_form: UserCreate) -> UserSchema:
        users_form = users_form_repo.create(db=db, obj_in=users_form)
        return users_form

    def update_users_form(self, *, db: Session, id: UUID, users_form: UserUpdate) -> UserSchema:
    ...
    def get_users_form(self, *, db: Session, id: UUID) -> UserSchema:
        users_form = users_form_repo.get(db=db, id=id)
        if not users_form:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="users_form not found")
        return users_form

    def delete_users_form(self, *, db: Session, id: UUID) -> UserSchema:
        users_form = users_form_repo.get(db=db, id=id)
        if not users_form:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="users_form not found")
    ...
    def get_users_form_by_id(self, *, id: UUID) -> UserSchema:
    ...
    def get_users_form_by_keywords(self, *, db: Session, tag: str) -> List[UserSchema]:
        pass

    def search_users_form(self, *, db: Session, search: str, value: str) -> List[UserSchema]:
        pass
...
users_form_service = UserService()