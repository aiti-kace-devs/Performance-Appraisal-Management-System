...
class RolePermssionService:
    ...
    # def create_roles_perm(self, *, role_perm: RolePermissionCreate, db:Session):

    #     # Check if the role name already exists
    #     existing_role = db.query(self.model).filter(self.model.name == obj_in.name).first()
        
    #     if existing_role:
    #         raise HTTPException(
    #             status_code=status.HTTP_400_BAD_REQUEST,
    #             detail=f"Role '{obj_in.name}' already exists."
    #         )

    #     db_role = self.model(name=obj_in.name)
    #     # obj_in_data = jsonable_encoder(obj_in, **kw)
    #     # db_obj = self.model(**obj_in_data)
    #     # print(db_role)
    #     # print(obj_in)
    #     for per in obj_in.permissions:
    #         try:
