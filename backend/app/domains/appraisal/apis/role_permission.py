...
# @role_perm_router.post("/assign-role-perm/", response_model=schema.ShowRolePerm,)
# def create_new_role_permission(*, payload: schema.CreateRolePerm, db:Session=Depends(get_db), dependencies=[Depends(check_if_is_super_admin)]):
#     new_role_perm = rolePermBaseActions.create_roles(payload, db)
...
## endpoint to 
@role_perm_router.get("/roles-perm/", response_model=List[schema.RolePermissionRead])
