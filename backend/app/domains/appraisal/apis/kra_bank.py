...
from domains.appraisal.services.kra_bank import kra_bank_service as actions
...
def list_appraisal_forms(
    ...
    kra_bank_router = actions.list_kra_bank(db=db, skip=skip, limit=limit)
...
def update_kra_bank(
    ...
    kra_bank_router = actions.get_kra_bank(db=db, id=id)
    if not kra_bank_router:
        raise HTTPException(
            status_code=HTTP_404_NOT_FOUND,
            detail="kra_bank_router not found"
        )
    kra_bank_router = actions.update_kra_bank(db=db, id=kra_bank_router.id, kra_bank_in_in=kra_bank_in)
    return kra_bank_router
...
def delete_kra_bank(
    ...
    kra_bank_router = actions.delete_kra_bank(db=db, id=id)
    return kra_bank_router
